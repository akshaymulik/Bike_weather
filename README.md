...
A. API to Bucket via Pub/Sub (Implemented on Docker and Airflow V2). 
 0. Pub/Sub helps in concurrent asynchronous operations and storing data in bucket. 
    This helps manage failures when we ingest to data lakes. 
...
B. Read from Bucket to Big Querry
  ...
  1. Airflow DAG will extract file name and bucket name to ingest the data in GCS for 
     further processing.
```
